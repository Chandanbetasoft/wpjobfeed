 ...
 */
function wpjf_process_application_form() {

	// store message on success/failure in this array.
	global $wpjf_messages, $post;
	$wpjf_messages = array();

	// check our nonce verifies.
	if ( ! isset( $_POST['wpjf_application_nonce_field'] ) || ! wp_verify_nonce( $_POST['wpjf_application_nonce_field'], 'wpjf_application_nonce_action' ) ) {

		// add an error message.
		$wpjf_messages['nonce_error'] = array(
			'type'    => 'error',
			'message' => __( 'Error: Nonce problem.', 'wpjobfeed' ),
		);
	...
	if ( ! isset( $_POST['wpjf_application_data'] ) ) {
		return;
	}

	// store our posted data array.
	$application_data = $_POST['wpjf_application_data'];
	...
	$application_post_id = wp_insert_post(
		apply_filters(
			'wpjf_insert_application_post_args',
			array(
				'post_type'   => 'wpjf_application',
	...
	if ( 0 === $application_post_id ) {
		...
		// add an error message.
		$wpjf_messages['application_not_saved'] = array(
			'type'    => 'error',
			'message' => __( 'Error: Application could not be saved.', 'wpjobfeed' ),
		);
		...
		$wpjf_messages['application_saved'] = array(
			'type'    => 'success',
			'message' => __( 'Success: Application received.', 'wpjobfeed' ),
		);
	...
	if ( ! empty( $fields ) ) {
		...
		foreach ( $fields as $field ) {
			...
			if ( 'file' === $field['input_type'] ) {
				...
				$attachment_ids[] = wpjf_action_file_upload_field( $application_post_id, $field );
				...
				if ( false === $field_added ) {
					...
					// add an error message.
					$wpjf_messages['application_field_not_saved'] = array(
						'type'    => 'error',
						'message' => sprintf(
							/* Translators: 1: Field name or label */
							__( 'Error: %s could not be added to the application.', 'wpjobfeed' ),
							$field['name']
						),
					);
	 ...
	 *
	 * @hooked wpjf_send_application_email_notification - 10.
	 * @hooked wpjf_remove_application - 20.
	 */
	do_action( 'wpjf_application_processing_complete', $application_post_id, $attachment_ids );

}

add_action( 'wp', 'wpjf_process_application_form' );
