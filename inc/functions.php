 ...
 */
function wpjf_job_post_type_name() {
	return apply_filters( 'wpjf_job_post_type_name', 'wpjf_job' );
}
 ...
 */
function wpjf_get_feed_username() {
	return esc_html( apply_filters( 'wpjf_feed_username', get_option( 'wpjf_username' ) ) );
}
 ...
 */
function wpjf_get_feed_password() {
	return esc_html( apply_filters( 'wpjf_feed_password', get_option( 'wpjf_password' ) ) );
}
 ...
 */
function wpjf_show_plugin_credit() {
	return apply_filters( 'wpjf_plugin_credit_status', (bool) get_option( 'wpjf_plugin_credit', false ) );
}
 ...
 */
function wpjf_hide_job_data_output() {
	return apply_filters( 'wpjf_hide_job_data_output', (bool) get_option( 'wpjf_hide_job_data_output', false ) );
}
 ...
 */
function wpjf_get_job_application_type() {
	return apply_filters( 'wpjf_job_application_type', get_option( 'wpjf_job_application_type', 'form' ) );
}
 ...
 */
function wpjf_get_plugin_settings( $settings_group = 'all' ) {

	// run the setting filter, where all the settings are added to.
	$settings = apply_filters( 'wpjf_plugin_settings', array() );
	...
	uasort( $settings, 'wpjf_array_sort_by_order_key' );
 ...
 */
function wpjf_get_settings_groups() {

	// store the output as an array.
	$output = array();

	// get all the registered settings.
	$registered_settings = wpjf_get_plugin_settings();
	...
	return apply_filters( 'wpjf_settings_groups', $output );
 ...
 */
function wpjf_get_job_applicant_tracking_email( $job_id = 0 ) {

	// return the tracking email from post meta.
	return apply_filters(
		'wpjf_job_applicant_tracking_email',
		get_post_meta( $job_id, '_wpjf_job_Jobfeed_application_email', true ),
		$job_id
	);
 ...
 */
function wpjf_get_job_applicant_tracking_url( $job_id = 0 ) {

	// return the tracking url from post meta.
	return apply_filters(
		'wpjf_job_applicant_tracking_url',
		get_post_meta( $job_id, '_wpjf_job_Jobfeed_application_url', true ),
		$job_id
	);
...
/**
 * Prepare terms sent via the Jobfeed feed to adding to a job post once created.
 ...
 *
 * @param (string) $sent_terms      The terms sent via the Jobfeed feed for this taxonomy.
 * @param (sstring) $taxonomy       The taxomony name associated with the terms in $sent_terms.
 * @return (array)  $wpjf_tax_terms An array with all the taxonomies terms.
 */
function wpjf_prepare_terms( $sent_terms, $taxonomy ) {
	...
	$wpjf_tax = wp_strip_all_tags( $sent_terms );
	$wpjf_taxonomy_terms = explode( '|', $wpjf_tax );

	// setup array to store the category term ids in.
	$wpjf_tax_terms = array();

	// loop through each term in array getting its id.
	foreach ( $wpjf_taxonomy_terms as $wpjf_taxonomy_term ) {
		...
		if ( $term_id = term_exists( $wpjf_taxonomy_term, $taxonomy['taxonomy_name'] ) ) {
			...
			if ( '' !== $wpjf_taxonomy_term ) {
				...
				if ( true === $taxonomy['hierarchical'] ) {
					...
					// add to term id to our terms array.
					$wpjf_tax_terms[] = $new_term['term_id'];
					...
					/* add the term name to our terms array */
					$wpjf_tax_terms[] = $new_term_obj->name;
...
function wpjf_get_input_field_wrapper_class( $field ) {
	...
	$classes[] = 'wpjf-col-' . $columns;

	// return the column class string.
	return apply_filters( 'wpjf_input_field_wrapper_class', implode( ' ', $classes ), $field );
 ...
 */
function wpjf_get_input_field_class( $field ) {
	...
	return apply_filters( 'wpjf_input_field_class', implode( ' ', $classes ), $field );
 ...
 */
function wpjf_get_input_field_name( $field ) {
	...
	return apply_filters( 'wpjf_input_field_name', $field['name'], $field );
 ...
 */
function wpjf_get_input_field_placeholder( $field ) {
	...
	return apply_filters( 'wpjf_input_field_placeholder', $field['placeholder'], $field );
 ...
 */
function wpjf_array_sort_by_second_level_order_key( $a, $b ) {
 ...
 */
function wpjf_get_job_by_reference( $job_ref = 0 ) {

	// run a WP_Query to find the job with this reference.
	$jobs = new WP_Query(
		apply_filters(
			'wpjf_get_job_by_reference_args',
			array(
				'post_type'      => wpjf_job_post_type_name(),
				'post_status'    => 'publish',
				'posts_per_page' => 750,
				'meta_key'       => '_wpjf_job_reference',
				'meta_value'     => $job_ref,
				'fields'         => 'ids',
				'no_found_rows'  => true,
			),
	...
	// return the job id.
	return apply_filters( 'wpjf_get_job_by_reference', array_shift( $jobs->posts ), $job_ref );
 ...
 */
function wpjf_load_view( $path = '', $data = array() ) {

	// build a filterable path for the view.
	$view_path = apply_filters( 'wpjf_load_view_path', WPJF_LOCATION . '/views/' . $path . '.php', $path, $data );
	...
	/* return the content */
	return apply_filters( 'wpjf_load_view_content', $content, $path, $data );
 ...
 */
function wpjf_get_job_currency_symbol( $post_id = 0 ) {

	// set a filterable default currency.
	$default_currency_symbol = apply_filters( 'wpjf_default_currency_symbol', '&pound;', $post_id );

	// get the currency code for this job.
	$currency_code = get_post_meta( $post_id, wpjf_get_job_field_prefix() . 'salary_currency', true );
	...
	return apply_filters( 'wpjf_get_job_currency_symbol', $currency_symbol, $post_id );
 ...
 */
function wpjf_get_job_meta_data( $args = array() ) {
	...
	$output .= '<div class="wpjf-job-data__wrapper">';

	// if we should output the title.
	if ( true === $args['show_title'] ) {

		// add the title next.
		$output .= '<h3 class="wpjf-job-data__title">' . esc_html__( 'Job Information', 'wproadbean' ) . '</h3>';

	}

	// add the job data div.
	$output .= '<div class="wpjf-job-data">';
	...
	if ( ! empty( $args['meta'] ) ) {
		...
		foreach ( $args['meta'] as $field ) {
			...
			if ( wpjf_get_job_field_prefix() . 'salary_from' === $field['id'] || wpjf_get_job_field_prefix() . 'salary_to' === $field['id'] ) {
				$value_prefix = wpjf_get_job_currency_symbol( $args['post_id'] );
			} else {
				$value_prefix = '';
			}

			// add this field to the output string.
			$meta_output .= '<div class="wpjf-job-data__field wpjf-job-data__field--' . esc_attr( str_replace( '_', '-', $field['xml_field'] ) ) . '" data-type="meta"><span class="label">' . esc_html( $field['name'] ) . ':</span> <span class="value">' .  $value_prefix . $field_value . '</span></div>';
	...
	if ( ! empty( $args['taxonomies'] ) ) {
		...
		foreach ( $args['taxonomies'] as $tax ) {
			...
			if ( '' !== $terms ) {
				...
				$tax_output .= '<div class="wpjf-job-data__field wpjf-job-data__field--' . esc_attr( str_replace( '_', '-', $tax['slug'] ) ) . '" data-type="term"><span class="label">' . esc_html( $tax['plural'] ) . ':</span> <span class="value">' .  $terms . '</span></div>';
	...
	if ( '' !== $output ) {
		...
		return apply_filters( 'wpjf_get_job_meta_data', $output . '</div></div>', $args );
 ...
 */
function wpjf_get_job_application_url( $job_id = 0 ) {

	// return the tracking email from post meta.
	return apply_filters(
		'wpjf_job_application_url',
		get_post_meta( $job_id, wpjf_get_job_field_prefix() . 'application_url', true ),
		$job_id
	);
 ...
 */
function wpjf_maybe_application_field_required( $field = array() ) {
 ...
 */
function wpjf_action_file_upload_field( $application_id = 0, $field = array() ) {
	...
	if ( ! is_array( $uploaded_file ) ) {
		...
		$wpjf_messages['attachment_failed'] = array(
			'type'    => 'error',
	...
	if ( '' !== $uploaded_file_name ) {
		...
		$allowed_mime_types = apply_filters(
			'wpjf_application_allowed_file_types',
			array(
				'pdf'       => 'application/pdf',
		...
		if ( ! in_array( $filetype['type'], $allowed_mime_types, true ) ) {
			...
			/* add an error message */
			$wpjf_messages['cv_type_failed'] = array(
				'type'    => 'error',
 ...
 */
function wpjf_text_html_email_type() {
	return 'text/html';
}
 ...
 */
function wpjf_get_application_applicant_name( $application_id = 0 ) {
	return apply_filters(
		'wpjf_application_applicant_name',
 ...
 */
function wpjf_get_application_applicant_email( $application_id = 0 ) {
	return apply_filters(
		'wpjf_application_applicant_name',
 ...
 * @return mixed    Either 0 if the values match or 1 or -1 if they are different.
 */
function wpjf_array_sort_by_order_key( $a, $b ) {
