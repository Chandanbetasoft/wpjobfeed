 ...
 */
function wpjf_save_job( $post_id, $post ) {
	...
	if ( empty( $_POST['wpjf_job_fields_nonce'] ) || ! wp_verify_nonce( $_POST['wpjf_job_fields_nonce'], 'wpjf_save_job_fields' ) ) {
		return; // do nothing.
	}
	...
	do_action( 'wpjf_save_job', $post_id, $post );

}

add_action( 'save_post_wpjf_job', 'wpjf_save_job', 1, 2 );
 ...
 * @param  obj $post    the post object for the current job being saved.
 */
function wpjf_save_job_fields( $post_id, $post ) {
		...
		switch ( $type ) {
			...
			default :
				...
				if ( '' === $type ) {
					...
					do_action( 'wpjf_save_field_type', $field, $post_id, $post );

				} else { // we have a type.

					// run a dynamic action based on the type.
					do_action( 'wpjf_save_field_type_' . $type, $field, $post_id, $post );
...
add_action( 'wpjf_save_job', 'wpjf_save_job_fields', 20, 2 );
