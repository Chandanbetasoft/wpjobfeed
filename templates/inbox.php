<?php
/**
 * This file is loaded when the endpoint /wpjf/jobfeed is reached.
 * This file can be overriden by copying it to your active theme in the following location /wpjf/inbox.php
 ...
 * you can use the wpjf_xml_feed_url filter
 */
...
if(isJson($xml_posted)){
	$xml = json_decode($xml_posted);
	$xml = isset($xml->job) ? $xml->job : $xml;
}else{
	$xml = simplexml_load_string($xml_posted);
}
//$xml = simplexml_load_string($xml_posted);

$compny_name = (string) $xml->contact_name;
$salary_per = (string) $xml->salary_per;
$show_salary = (string) $xml->show_salary;
$_custom_application_url = (string) $xml->application_url;
/*$mypost = get_page_by_title( $xml->contact_name, '', 'noo_job' );*/
//$mypost = get_post_by_title($compny_name);
...
$creds = array(
	'user_login'    => (string) $xml->username,
	'user_password' => (string) $xml->password,
	'remember'      => false
);
...
// if the username and or password don't match.
	if ( (string) $xml->username !== $username || (string) $xml->password !== $password ) {
		...
		esc_html_e( 'Authorization failed. Incorrect username or password.', 'wpjobfeed' );
		die();
...
$allowed_roles = array( 'editor', 'administrator' ); // Define allowed roles
$user_roles = $user->roles;
$intersect_roles = array_intersect( $allowed_roles, $user_roles ); // Get intersecting roles
if ( empty( $intersect_roles ) ) {
    // User does not have required role, send 403 Forbidden response
    header( 'HTTP/1.1 403 Forbidden' );
    esc_html_e( 'You do not have permission to access this resource.', 'wpjobfeed' );
    die();
}
...
$update_post_id = wpjf_get_job_by_reference( $job_reference );
...
if ( 'add' === $command ) {
	...
	if ( 0 === $job_id ) {
		...
		esc_html_e( 'All looked good, but for some reason the job was not added.', 'wpjobfeed' );
		die();
	...
	if ( ! empty( $job_fields ) ) {
		...
		foreach ( $job_fields as $job_field ) {
			...
			if ( false !== $job_field_added ) {
				 ...
				 */
				do_action( 'wpjf_inbox_job_field_added', $job_id, $job_field['id'], $field_value, $xml );
	...
	if ( ! empty( $taxonomies ) ) {
		...
		foreach ( $taxonomies as $taxonomy ) {
			...
			$job_tax_terms[ $taxonomy['xml_field'] ] = wpjf_prepare_terms( $xml->{ $taxonomy['xml_field'] }, $taxonomy );
			...
			if ( ! empty( $job_tax_terms ) ) {
				...
				if ( false !== $term_added && ! is_wp_error( $term_added ) ) {
					 ...
					 * @param (string) $wpjf_term term to be added
					 * @param (string) $taxonomy taxonomy of the term
					 */
					do_action( 'wpjf_inbox_job_term_added', $job_id, $job_tax_terms[ $taxonomy['xml_field'] ], $taxonomy['taxonomy_name'], $xml );
	 ...
	 */
	do_action( 'wpjf_inbox_job_added', $job_id, $xml );
	...
	$text = sprintf(
		...
		__( 'Success! Job added/updated with a post ID of %1s. You can view the job here: %2s', 'wpjobfeed' ),
		$job_id,
	...
	if ( empty( $update_post_id ) ) {
		...
		// echo an error.
		header( 'HTTP/1.1 406 Not Acceptable' );
		esc_html_e( 'Sorry, a job with that job reference could not be found.', 'wpjobfeed' );
		die();
	...
	if ( false === $deleted ) {
		...
		/* echo an error */
		header( 'HTTP/1.1 406 Not Acceptable' );
		esc_html_e( 'Error: job was not deleted. Adding to trash failed.', 'wpjobfeed' );
		die();
		 ...
		 */
		do_action( 'wpjf_inbox_job_deleted', $update_post_id, $xml );
	...
	esc_html_e( 'Invalid command sent.', 'wpjobfeed' );
	die();
